"""
老版本 半废弃
相较于上版本 curLink 输出markdown结果，并保存prob_adj结果绘制结果图用
"""
...
# curlink的主函数
def main_of_curLink(num_u, num_epoch, R_ball, hyperBip_id_vec_list, social_id_vec_list, friendship_old_df, friendship_new_df, NUM_diff_leve,link_new_labels_M):
    #print("-" * 10, "计算user的学习困难程度", "-" * 10)
    # 首先计算user的学习困难程度
    node_diffASC_list = count_diff_of_nodes(num_u, R_ball, social_id_vec_list, friendship_old_df)

    # 构建训练的data
    x_curLink = torch.tensor(np.array(hyperBip_id_vec_list), dtype=torch.float32) # 先转array再转tensor更快
    edge_index = [[], []]
    for uid1, uid2 in zip(friendship_old_df['uid1'], friendship_old_df['uid2']):
    ...
    prob_adj = zzz @ zzz.t()
    # prob_adj = - abs(1 - prob_adj) # 距离1越近值越大
...
# 异构超图+GAT的主函数
def main_of_HBGAT(num_u, num_epoch, HeterHyper_id_vec_list, friendship_old_df, friendship_new_df,link_new_labels_M):
    # 构建训练的data
...
def main_of_baseline(K, hyperBip_id_vec_list, social_id_vec_list, HeterHyper_id_vec_list,link_new_labels_M):
    result_baseline = {}
    prob_adj_set = {}
    # # 超二部图结果
    print("-" * 10, "超二部图结果", "-" * 10)
...
if __name__ == '__main__':
    ...
    for city in cities:
        ...
        result_city, prob_adj_city = main_of_baseline(10, hyperBip_id_vec_list, social_id_vec_list, HeterHyper_id_vec_list, link_new_labels_M) # For Markdown
        ...
        P_K, R_K, F1, prob_adj = main_of_HBGAT(num_u, num_epoch, HeterHyper_id_vec_list, friendship_old_df, friendship_new_df, link_new_labels_M)
        ...
        # num_epoch = 500 # 学习epoch数
        #
        # P_K, R_K, F1, prob_adj = main_of_curLink(num_u, num_epoch, R_ball, HeterHyper_id_vec_list, HeterHyper_id_vec_list, friendship_old_df, friendship_new_df,
        #                 NUM_diff_leve, link_new_labels_M)
        ...
        P_K, R_K, F1, prob_adj = main_of_curLink(num_u, num_epoch, R_ball, hyperBip_id_vec_list, social_id_vec_list, friendship_old_df,
                        friendship_new_df, NUM_diff_leve, link_new_labels_M)
