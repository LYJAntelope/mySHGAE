...
def load_params():
    ...
    return args, kwargs, top_K, negative_candidates, GAT_dimension
...
def main_of_SHGAE(num_u, HeterHyper_all_id_vec_list, friendship_old_df, bipG_t, bipG_v, bipG_c):
    ...
    model = GATNet(num_node_features, GAT_dimension).to(device)
...
if __name__ == '__main__':
    args, kwargs, top_K, negative_candidates, GAT_dimension = load_params()
    cities = ["NYC", "SaoPaulo"]
    for city in cities:
        ...
        num_u, num_t, num_v, num_c, bipG_t, bipG_v, bipG_c, social_G = get_Graph(friendship_old_df, checkins_df, offset_DIC)
        num_all_nodes = num_u + num_t + num_v + num_c + 1
